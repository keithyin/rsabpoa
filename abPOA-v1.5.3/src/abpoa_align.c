...
{
    int n, is_base = 1, _i = -1;
    long s;
    char *str = l, *pEnd = NULL;
    for (n = 0; *str; ++str)
    {
        ...
        { // get base
            ...
            if (_i >= m)
                err_fatal(__func__, "Unknown base: \"%c\" (%d).\n", *str, _i);
            is_base = 0;
        }
        else
        { // get score
...
{
    char *l = (char *)_err_malloc(1024 * sizeof(char));
    FILE *fp;
    if ((fp = fopen(mat_fn, "r")) == NULL)
        err_fatal(__func__, "Unable to open scoring matrix file: \"%s\"\n", mat_fn);
    int first_line = 1;
...
{
    if (abpt->gap_open1 == 0)
        abpt->gap_mode = ABPOA_LINEAR_GAP;
    else if (abpt->gap_open1 > 0 && abpt->gap_open2 == 0)
...
void abpoa_post_set_para(abpoa_para_t *abpt)
{
    ...
    {
        abpt->use_read_ids = 1;
        if (abpt->out_msa || abpt->out_gfa)
            set_65536_table();
        if (abpt->max_n_cons > 1 || abpt->cons_algrm == ABPOA_MF)
            set_bit_table16();
    }
    ...
    if (abpt->use_score_matrix == 0)
        gen_simple_mat(abpt);
    else
        abpoa_set_mat_from_file(abpt, abpt->mat_fn);
}
...
void abpoa_free_para(abpoa_para_t *abpt)
{
    ...
    if (abpt->mat_fn != NULL)
        free(abpt->mat_fn);
    if (abpt->out_pog != NULL)
        free(abpt->out_pog);
    if (abpt->incr_fn != NULL)
        free(abpt->incr_fn);
    free(abpt);
}
...
int abpoa_align_sequence_to_subgraph(abpoa_t *ab, abpoa_para_t *abpt, int exc_beg_node_id, int exc_end_node_id, uint8_t *query, int qlen, abpoa_res_t *res)
{
    if (ab->abg->node_n <= 2)
        return -1;
    if (ab->abg->is_topological_sorted == 0)
...
int abpoa_anchor_poa(abpoa_t *ab, abpoa_para_t *abpt, uint8_t **seqs, int **weights, int *seq_lens, ab_u64_v par_anchors, int *par_c, int *tpos_to_node_id, int *qpos_to_node_id, int *read_id_map, int exist_n_seq, int n_seq)
{
    ...
    abpoa_res_t res;
    int read_id, last_read_id = -1, m_c = 0, k = abpt->k, qlen;
    abpoa_seq_t *abs = ab->abs;
    int *tmp;
    int i, _i, ai, j, tot_n_seq = exist_n_seq + n_seq;
    uint8_t *qseq;
    int *weight;
    abpoa_res_t whole_res;
    // uint8_t *seq1;
    for (_i = 0; _i < n_seq; ++_i)
    {
        ...
        qlen = seq_lens[i];
        whole_res.n_cigar = 0, whole_res.m_cigar = 0, whole_res.graph_cigar = 0;
        ...
        if (ai < par_c[_i])
        {
            ...
            if (abs->is_rc[read_id])
            {
                ...
                for (j = 0; j < qlen; ++j)
                {
                    if (seqs[i][qlen - j - 1] < 4)
                        qseq[j] = 3 - seqs[i][qlen - j - 1];
                    else
                        qseq[j] = 4;
                    weight[j] = weights[i][qlen - j - 1];
                }
                if (abs->is_rc[last_read_id])
                { // reset tpos/qpos in par_anchors
                    int last_qlen = seq_lens[read_id_map[_i - 1]];
                    for (j = ai; j < par_c[_i]; ++j)
                    {
                        end_tpos = ((par_anchors.a[j] >> 32) & 0x7fffffff);
                        end_qpos = (uint32_t)par_anchors.a[j];
        ...
        {
            abs->is_rc[read_id] = 0, qseq = seqs[i];
            weight = weights[i];
        }

        for (; ai < par_c[_i]; ++ai)
        {
            end_tpos = ((par_anchors.a[ai] >> 32) & 0x7fffffff) - k + 1;
            end_id = tpos_to_node_id[end_tpos];
            ...
            res.graph_cigar = (abpoa_cigar_t *)_err_malloc((k) * sizeof(abpoa_cigar_t));
            res.n_cigar = 0;
            m_c = k;
            for (j = 0; j < k; ++j)
            ...
            // for next anchor
            beg_id = tpos_to_node_id[end_tpos + k - 1];
            beg_qpos = end_qpos + k;
        }
        end_id = ABPOA_SINK_NODE_ID;
        end_qpos = seq_lens[i];
        ...
        if (whole_res.n_cigar)
            free(whole_res.graph_cigar);

        tmp = qpos_to_node_id;
        qpos_to_node_id = tpos_to_node_id;
        tpos_to_node_id = tmp;
...
int abpoa_poa(abpoa_t *ab, abpoa_para_t *abpt, uint8_t **seqs, int **weights, int *seq_lens, int exist_n_seq, int n_seq)
{
    ...
    abpoa_seq_t *abs = ab->abs;
    abpoa_res_t res;
    int i, j, read_id, qlen, tot_n_seq = exist_n_seq + n_seq;
    uint8_t *qseq, *rc_qseq;
    int *weight, *rc_weight;
    // uint8_t *seq1;
    for (i = 0; i < n_seq; ++i)
    {
        qlen = seq_lens[i];
        ...
        {
            if (abpt->amb_strand && (res.best_score < MIN_OF_TWO(qlen, ab->abg->node_n - 2) * abpt->max_mat * .3333))
            { // TODO .3333
                rc_qseq = (uint8_t *)_err_malloc(sizeof(uint8_t) * qlen);
                for (j = 0; j < qlen; ++j)
                {
                    if (qseq[qlen - j - 1] < 4)
                        rc_qseq[j] = 3 - qseq[qlen - j - 1];
                    else
                        rc_qseq[j] = 4;
                }
                ...
                abpoa_res_t rc_res;
                rc_res.n_cigar = 0, rc_res.graph_cigar = 0;
...
void abpoa_output(abpoa_t *ab, abpoa_para_t *abpt, FILE *out_fp)
{
    // generate & output GFA
    if (abpt->out_gfa)
        abpoa_generate_gfa(ab, abpt, out_fp);
    else
    {
        ...
        // output cons/rc-msa
        if (abpt->out_msa)
            abpoa_output_rc_msa(ab, abpt, out_fp);
        else if (abpt->out_cons)
            abpoa_output_fx_consensus(ab, abpt, out_fp);
    }
...
{
    if ((!abpt->out_msa && !abpt->out_cons && !abpt->out_gfa) || n_seq <= 0)
        return 0;
    abpoa_reset(ab, abpt, 1024);
    if (abpt->incr_fn)
        abpoa_restore_graph(ab, abpt); // restore existing graph
    abpoa_seq_t *abs = ab->abs;
    int i, exist_n_seq = abs->n_seq;

    // set ab->abs, name
    // printf("before:abs->n_seq:%d\n", abs->n_seq);
    abs->n_seq += n_seq;
    // printf("after:abs->n_seq:%d\n", abs->n_seq);
    ...
    else
    {
        for (i = 0; i < n_seq; ++i)
        {
            abs->name[exist_n_seq + i].l = 0;
            abs->name[exist_n_seq + i].m = 0;
        }
    }
    ...
    for (i = 0; i < n_seq; ++i)
    {
        ...
        if (abpt->use_qv && qual_weights != NULL && qual_weights[i] != NULL)
        {
            for (j = 0; j < seq_lens[i]; ++j)
                weights[i][j] = (int)qual_weights[i][j];
        }
        else
        {
            for (j = 0; j < seq_lens[i]; ++j)
    ...
    else
    {
        ...
        ab_u64_v par_anchors = {0, 0, 0};
        int *par_c = (int *)_err_calloc(n_seq, sizeof(int));
        ...
        free(read_id_map);
        free(tpos_to_node_id);
        free(qpos_to_node_id);
        free(par_c);
        if (par_anchors.m > 0)
            free(par_anchors.a);
    }
    ...
    for (i = 0; i < n_seq; ++i){
        // printf("freeing... %d / %d, ptr:%p\n", i, n_seq, weights[i]);
        // double free 不是这的问题！????
        free(weights[i]);
    };
    // printf("free single weight done\n");
...
{
    if (!abpt->out_msa && !abpt->out_cons && !abpt->out_gfa)
        return 0;
    abpoa_reset(ab, abpt, 1024);
    if (abpt->incr_fn)
        abpoa_restore_graph(ab, abpt); // restore existing graph
    ...
    {
        if (abs->seq[i].l > max_len)
            max_len = abs->seq[i].l;
    }

    // set seqs, seq_lens
    extern char ab_char26_table[256];
    uint8_t **seqs = (uint8_t **)_err_malloc(n_seq * sizeof(uint8_t *));
    int *seq_lens = (int *)_err_malloc(n_seq * sizeof(int));
    ...
    for (i = 0; i < n_seq; ++i)
    {
        ...
        for (j = 0; j < seq_lens[i]; ++j)
            seqs[i][j] = ab_char26_table[(int)abs->seq[exist_n_seq + i].s[j]];
        if (abpt->use_qv && abs->qual[exist_n_seq + i].l > 0)
        {
            for (j = 0; j < seq_lens[i]; ++j)
                weights[i][j] = (int)abs->qual[exist_n_seq + i].s[j] - 32;
        }
        else
        {
            for (j = 0; j < seq_lens[i]; ++j)
    ...
    else
    {
        ...
        abpoa_build_guide_tree_partition(seqs, seq_lens, n_seq, abpt, read_id_map, &par_anchors, par_c);
        if (abpt->incr_fn)
        { // TODO collect anchors between last one path and first seq
            // anchors
            // new_par_anchors
            // push anchors
            // free(par_anchors.a);
